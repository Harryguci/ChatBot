...
import { BiRefresh, BiTrash, BiUpload } from "react-icons/bi";
...
const DocumentManagementPage = () => {
  ...
  const getFileIcon = (filename: string) => {
    ...
    if (ext === "pdf") {
      return <FilePdfOutlined style={{ fontSize: "16px" }} />;
    }
    return <FileImageOutlined style={{ fontSize: "16px" }} />;
  };
      ...
      <div className="bg-white rounded-lg shadow p-6 mb-6">
        ...
        {replaceMode && (
          <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded">
            <p className="text-sm text-yellow-800">
              You are in replace mode. Upload a new file to replace "
              {replaceMode}".
            </p>
            <button
        ...
        <div className="flex space-y-4">
          ...
          <Button
            ...
            className="px-4 py-2 rounded-lg hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed mb-0 mt-auto"
            type="primary"
            icon={<BiUpload />}
            size="large"
          >
                ...
                {documents.map((doc, index) => (
                  <tr key={index} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <Tooltip
                        title={doc.filename.length > 30 ? doc.filename : ""}
                      >
